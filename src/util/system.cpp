...
const char * const MONEDADELMUNDO_CONF_FILENAME = "monedadelmundo.conf";
const char * const MONEDADELMUNDO_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //monedadelmundo-tx using stdin
        std::string val;
...
{
    fs::path settings = GetPathArg("-settings", fs::path{MONEDADELMUNDO_SETTINGS_FILENAME});
    if (settings.empty()) {
        return false;
    }
    if (filepath) {
...
#else
    const char* pszModule = "monedadelmundo";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\MonedaDelMundo
    // macOS: ~/Library/Application Support/MonedaDelMundo
    // Unix-like: ~/.monedadelmundo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MonedaDelMundo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/MonedaDelMundo";
#else
    // Unix-like
    return pathRet / ".monedadelmundo";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MONEDADELMUNDO_CONF_FILENAME);
