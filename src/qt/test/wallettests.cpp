...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<MonedaDelMundoAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_monedadelmundo-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_monedadelmundo-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_monedadelmundo-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = MonedaDelMundoUnits::formatWithUnit(unit, balance, false, MonedaDelMundoUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    MonedaDelMundoAmountField* amountInput = receiveCoinsDialog.findChild<MonedaDelMundoAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("monedadelmundo:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_monedadelmundo-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
