...
#include <qt/monedadelmundoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MonedaDelMundo address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MonedaDelMundoAddressEntryValidator(parent));
    widget->setCheckValidator(new MonedaDelMundoAddressCheckValidator(parent));
}
...
bool parseMonedaDelMundoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no monedadelmundo: URI
    if(!uri.isValid() || uri.scheme() != QString("monedadelmundo"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!MonedaDelMundoUnits::parse(MonedaDelMundoUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseMonedaDelMundoURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMonedaDelMundoURI(uriInstance, out);
}

QString formatMonedaDelMundoURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("monedadelmundo:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MonedaDelMundoUnits::format(MonedaDelMundoUnit::BTC, info.amount, false, MonedaDelMundoUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMonedaDelMundoConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MONEDADELMUNDO_CONF_FILENAME));
    ...
    /* Open monedadelmundo.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MonedaDelMundo.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MonedaDelMundo (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("MonedaDelMundo (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for MonedaDelMundo*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "monedadelmundo.desktop";
    return GetAutostartDir() / strprintf("monedadelmundo-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a monedadelmundo.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=MonedaDelMundo\n";
        else
            optionFile << strprintf("Name=MonedaDelMundo (%s)\n", chain);
