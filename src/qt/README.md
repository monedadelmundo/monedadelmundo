This directory contains the source code for the MonedaDelMundo Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### monedadelmundogui.(h/cpp)

- Represents the main window of the MonedaDelMundo UI.
...
- ClientModel is used by the main application `monedadelmundogui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a monedadelmundo:... link from a browser).
...
* UI elements like MonedaDelMundoAmountField, which inherit from QWidget.
* `monedadelmundostrings.cpp`: automatically generated
* `monedadelmundounits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/monedadelmundo_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `monedadelmundo-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `monedadelmundo-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `monedadelmundo-qt`
9. You're all set! Start developing, building, and debugging the MonedaDelMundo Core GUI
