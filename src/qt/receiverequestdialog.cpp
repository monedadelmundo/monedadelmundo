...
#include <config/monedadelmundo-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatMonedaDelMundoURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(MonedaDelMundoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatMonedaDelMundoURI(info));
}
