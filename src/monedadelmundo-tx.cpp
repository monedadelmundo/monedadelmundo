...
static void SetupMonedaDelMundoTxArgs(ArgsManager &argsman)
{
...
{
    SetupMonedaDelMundoTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " monedadelmundo-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  monedadelmundo-tx [options] <hex-tx> [commands]  Update hex-encoded monedadelmundo transaction\n"
                "or:     monedadelmundo-tx [options] -create [commands]   Create hex-encoded monedadelmundo transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded monedadelmundo transaction
