...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  CI_EXEC "${TEST_RUNNER_ENV}" DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}/bin/test_monedadelmundo" --catch_system_errors=no -l test_suite
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  export P_CI_DIR="${BASE_BUILD_DIR}/monedadelmundo-$HOST/src/"
