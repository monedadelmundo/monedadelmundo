Building MonedaDelMundo Core with Visual Studio
========================================

Introduction
---------------------
Solution and project files to build MonedaDelMundo Core with `msbuild` or Visual Studio can be found in the `build_msvc` directory. The build has been tested with Visual Studio 2019 (building with earlier versions of Visual Studio should not be expected to work).

To build MonedaDelMundo Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build MonedaDelMundo Core with the GUI, a static build of Qt is required.
...
To build MonedaDelMundo Core without Qt, unload or disable the `monedadelmundo-qt`, `libmonedadelmundo_qt` and `test_monedadelmundo-qt` projects.
...
```cmd
msbuild -property:Configuration=Release -maxCpuCount -verbosity:minimal monedadelmundo.sln
```

Alternatively, open the `build_msvc/monedadelmundo.sln` file in Visual Studio 2019.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make MonedaDelMundo Core more secure. When building MonedaDelMundo using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `monedadelmundod` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/monedadelmundod.exe
```
