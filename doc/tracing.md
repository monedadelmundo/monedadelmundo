# User-space, Statically Defined Tracing (USDT) for MonedaDelMundo Core

MonedaDelMundo Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ monedadelmundod     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: MonedaDelMundo Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to MonedaDelMundo Core
...
Multiple tools can list the available tracepoints in a `monedadelmundod` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in MonedaDelMundo Core, use `info probes` in `gdb`:

```
$ gdb ./src/monedadelmundod
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/monedadelmundod
stap net        outbound_message 0x0000000000107c05 /src/monedadelmundod
stap validation block_connected  0x00000000002fb10c /src/monedadelmundod
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in MonedaDelMundo Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/monedadelmundod | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/monedadelmundod -v
