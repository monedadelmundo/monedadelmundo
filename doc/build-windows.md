...
Below are some notes on how to build MonedaDelMundo Core for Windows.

The options known to work for building MonedaDelMundo Core on Windows are:
...
Note that for WSL the MonedaDelMundo Core source path MUST be somewhere in the default mount file system, for
example /usr/src/monedadelmundo, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\monedadelmundo`, for example:

    make install DESTDIR=/mnt/c/workspace/monedadelmundo
