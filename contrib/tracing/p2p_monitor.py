...
""" Interactive monedadelmundod P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for MonedaDelMundo Core can enable. It uses BCC
...
def main(monedadelmundod_path):
    peers = dict()

    monedadelmundod_with_usdts = USDT(path=str(monedadelmundod_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    monedadelmundod_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    monedadelmundod_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[monedadelmundod_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/monedadelmundod")
        exit()
    path = sys.argv[1]
    main(path)
