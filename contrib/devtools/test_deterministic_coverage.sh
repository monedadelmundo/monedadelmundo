...
TEST_MONEDADELMUNDO_BINARY="src/test/test_monedadelmundo"
...
if [[ ! -e ${TEST_MONEDADELMUNDO_BINARY} ]]; then
    echo "Error: Executable ${TEST_MONEDADELMUNDO_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_MONEDADELMUNDO_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
