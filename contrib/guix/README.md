# Bootstrappable MonedaDelMundo Core Builds

This directory contains the files necessary to perform bootstrappable MonedaDelMundo
Core builds.
...
To build MonedaDelMundo Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      MonedaDelMundo Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/monedadelmundo-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/monedadelmundo-core/guix.sigs).
