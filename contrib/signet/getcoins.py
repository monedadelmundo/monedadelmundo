...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing monedadelmundo-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='monedadelmundo-cli', help='monedadelmundo-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='MonedaDelMundo address to which the faucet should send')
...
parser.add_argument('monedadelmundo_cli_args', nargs='*', help='Arguments to pass on to monedadelmundo-cli (default: -signet)')

args = parser.parse_args()

if args.monedadelmundo_cli_args == []:
    args.monedadelmundo_cli_args = ['-signet']


def monedadelmundo_cli(rpc_command_and_params):
    argv = [args.cmd] + args.monedadelmundo_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = monedadelmundo_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = monedadelmundo_cli(['getnewaddress', 'faucet', 'bech32'])
